"""Freebuff Integration — execute coding tasks via Freebuff API/CLI."""
...
class FreebuffAgent:
    """Freebuff API/CLI integration for autonomous coding tasks."""

    def __init__(self):
        self._executions: Dict[str, FreebuffExecution] = {}
        self._lock = threading.Lock()
        self._freebuff_path = self._find_freebuff()
        self._api_key = os.environ.get("FREEBUFF_API_KEY")
    ...
    def _find_freebuff(self) -> Optional[str]:
        return shutil.which("freebuff")

    def is_available(self) -> bool:
        return self._freebuff_path is not None and self._api_key is not None

    def health(self) -> Dict[str, Any]:
        return {
            "name": self.name,
            "available": self.is_available(),
            "executable": self._freebuff_path,
    ...
    def inspect(self, workspace: str) -> Dict[str, Any]:
        if not self.is_available():
            return {"ok": False, "error": "Freebuff not installed or API key missing"}
    ...
    def start_task(self, task: Dict[str, Any], workspace: str, permissions: List[str]) -> str:
        if not self.is_available():
            raise RuntimeError("Freebuff not available")

        execution_id = f"freebuff-{uuid.uuid4().hex[:8]}"
        workspace_path = Path(workspace).resolve()

        task_description = task.get("description", task.get("title", ""))

        task_file = Path(workspace) / f".elysia_freebuff_task_{uuid.uuid4().hex[:8]}.md"
        task_file.write_text(f"# Task\n\n{task_description}\n")

        cmd = [self._freebuff_path, "run", "--task", str(task_file), "--workspace", str(workspace)]

        env = os.environ.copy()
        if self._api_key:
            env["FREEBUFF_API_KEY"] = self._api_key
...
def _discover_integrations(registry) -> None:
    """Register Freebuff if available."""
    try:
        agent = FreebuffAgent()
